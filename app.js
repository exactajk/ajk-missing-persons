...
function initWizardForm() {
    ...
    const uploadPlaceholder = dropzone.querySelector('.sf-upload-placeholder');
    const previewContainer = dropzone.querySelector('.sf-upload-preview-container');
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            const fatherName = ''; // Removed from form as per image request
            ...
            const village = document.getElementById('person-location').value.trim();
            const address = village + ", " + district;
            
            const date = document.getElementById('event-date').value;
            const time = ''; // Removed from form
            const lastSeen = village;
            ...
            imageUploadPromise.then((imageUrl) => {
                ...
                renderAllViews();
                setTimeout(() => {
                    navigateToView('profile'); // Send them to profile page where they see their submitted report in Pending state
                }, 1000);
            }).catch((err) => {
...
function applyLanguage(lang) {
    ...
    if (lang === 'ur') {
        ...
        setT('.sf-upload-sub-text', 'JPG، PNG یا WEBP (زیادہ سے زیادہ 1 MB)');
        ...
        setT('.sf-upload-sub-text', 'JPG, PNG or WEBP (Max 1 MB)');
