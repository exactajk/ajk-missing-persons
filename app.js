...
function renderHomeViews() {
    ...
    // 2. Recently Found (horizontal scroll cards)
    if (foundContainer) {
        const foundRecords = appState.records.filter(r => r.status === 'found' && r.approved === true).slice(0, 8);
    ...
    // 3. Shaheed Memorial (horizontal scroll cards)
    if (shaheedContainer) {
        const shaheedRecords = appState.records.filter(r => r.status === 'shaheed' && r.approved === true).slice(0, 8);
