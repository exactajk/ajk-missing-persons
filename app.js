...
function initAppState() {
    ...
    // 2. Fetch reports from Firebase Firestore
    db.collection('reports').get()
        .then((querySnapshot) => {
            ...
            if (tempRecords.length > 0) {
                ...
                appState.records = tempRecords;
                localStorage.setItem('ajk_portal_records', JSON.stringify(appState.records));
                renderAllViews();
            } else {
                // If firestore is empty, seed it with default records
                console.log("Seeding Firestore with default records...");
                const seedPromises = DEFAULT_RECORDS.map(record => {
                    return db.collection('reports').doc(record.id).set(record);
                });
        ...
        .catch((error) => {
            console.error("Firestore error, loading from local storage fallback:", error);
...
function renderHomeViews() {
    ...
    const missingRecords = appState.records.filter(r => r.status === 'missing' && r.approved === true).slice(0, 5);
    ...
    const foundRecords = appState.records.filter(r => r.status === 'found' && r.approved === true).slice(0, 5);
    ...
    // 3. Shaheed Memorial
    const shaheedRecords = appState.records.filter(r => r.status === 'shaheed' && r.approved === true).slice(0, 5);
...
function initWizardForm() {
    ...
    document.querySelectorAll('.type-option input').forEach(radio => {
        radio.addEventListener('change', () => {
            ...
            if (type === 'missing') {
                ...
                dateLabel.innerHTML = 'Date of Shahadat <span class="required">*</span>';
            } else {
                dateLabel.innerHTML = 'Date Found <span class="required">*</span>';
            }
        });
    ...
    form.addEventListener('submit', (e) => {
        ...
        const cnic = document.getElementById('person-cnic').value.trim();
        ...
        const isDuplicateCnic = cnic && appState.records.some(r => r.cnic === cnic && !r.rejected);
        const isDuplicateNamePhone = appState.records.some(r => 
            r.name.toLowerCase() === name.toLowerCase() && 
            r.reporterPhone === reporterPhone &&
            !r.rejected
        );
        ...
        if (isDuplicateNamePhone) {
            showToast('A report with the same name and reporter contact has already been filed!', 'error');
            return;
        }
        ...
        // Create new record skeleton
        const recordId = reportType[0] + Date.now().toString(36);
        const newRecord = {
            id: recordId,
            name,
            age,
            ...
            date,
            details,
            image: 'assets/images/logo.png', // Fallback default
            reporterName,
            reporterPhone,
            reporterRelation,
            userSubmitted: true,
            approved: false, // Must be approved by Admin to be public
            rejected: false,
            reporterUid: appState.currentUser ? appState.currentUser.uid : ''
        };

        // Firebase Storage image upload
        let imageUploadPromise = Promise.resolve('assets/images/logo.png');
        if (uploadedImageBase64) {
            // Upload to Firebase Storage
            const storageRef = storage.ref().child(`reports/${recordId}.jpg`);
            imageUploadPromise = storageRef.putString(uploadedImageBase64, 'data_url')
                .then((snapshot) => snapshot.ref.getDownloadURL())
                .catch((err) => {
                    console.error("Storage upload failed, saving as direct base64 in Firestore instead:", err);
                    // If storage upload fails, return base64 string directly
        ...
        imageUploadPromise.then((imageUrl) => {
            ...
            // Save to Firestore
            return db.collection('reports').doc(recordId).set(newRecord);
        }).then(() => {
            ...
            submitBtn.innerHTML = '<i data-lucide="check"></i> Submit Report';
            lucide.createIcons();
...
function initAdminPanel() {
    ...
    function renderAdminDashboard() {
        ...
        document.getElementById('admin-stat-total').innerText = pendingCount; // Use total pending as primary target
        document.getElementById('admin-stat-total').previousElementSibling.innerText = pendingCount;
        document.getElementById('admin-stat-total').nextElementSibling.innerText = "Pending Review";
        
        document.getElementById('admin-stat-missing').innerText = approvedCount;
        document.getElementById('admin-stat-missing').nextElementSibling.innerText = "Approved/Public";

        document.getElementById('admin-stat-found').innerText = rejectedCount;
        document.getElementById('admin-stat-found').nextElementSibling.innerText = "Rejected";
        ...
        // ─── Bind Approve Event ───
        adminReportsContainer.querySelectorAll('.admin-approve-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const recordId = e.target.getAttribute('data-id');
                db.collection('reports').doc(recordId).update({ approved: true, rejected: false })
                    .then(() => {
        ...
        // ─── Bind Reject Event ───
        adminReportsContainer.querySelectorAll('.admin-reject-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const recordId = e.target.getAttribute('data-id');
                db.collection('reports').doc(recordId).update({ approved: false, rejected: true })
                    .then(() => {
        ...
        // ─── Bind Status Dropdown Change ───
        adminReportsContainer.querySelectorAll('.admin-status-select').forEach(select => {
            select.addEventListener('change', (e) => {
                const recordId = e.target.getAttribute('data-id');
                const newStatus = e.target.value;

                db.collection('reports').doc(recordId).update({ status: newStatus })
                    .then(() => {
        ...
        // ─── Bind Close Case Event ───
        adminReportsContainer.querySelectorAll('.admin-close-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (closureDetails === null) return; // Cancel clicked
        ...
        // ─── Bind Delete Event ───
        adminReportsContainer.querySelectorAll('.admin-del-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const recordId = e.currentTarget.getAttribute('data-id');
                if (confirm("Are you sure you want to permanently delete this report?")) {
                    db.collection('reports').doc(recordId).delete()
                        .then(() => {
...
const TRANSLATIONS = {
  en: {
    ...
    lbl_person_cnic: "CNIC Number (Optional)",
  ...
  ur: {
    ...
    lbl_person_cnic: "شناختی کارڈ نمبر (اختیاری)",
...
function applyLanguage(lang) {
    ...
    if (lang === 'ur') {
        ...
        if (progSteps[0] && progSteps[0].querySelector('span')) progSteps[0].querySelector('span').innerText = 'رپورٹ کی قسم';
        if (progSteps[1] && progSteps[1].querySelector('span')) progSteps[1].querySelector('span').innerText = 'شخصی معلومات';
        if (progSteps[2] && progSteps[2].querySelector('span')) progSteps[2].querySelector('span').innerText = 'رپورٹر معلومات';
        
        setT('.wizard-step-content[data-step="1"] h3', 'رپورٹ کی قسم منتخب کریں');
        const typeOpts = document.querySelectorAll('.type-option');
        if (typeOpts[0]) {
            if (typeOpts[0].querySelector('h4')) typeOpts[0].querySelector('h4').innerText = 'لاپتہ فرد';
            if (typeOpts[0].querySelector('p')) typeOpts[0].querySelector('p').innerText = 'وہ شخص جو لاپتہ ہو اور تلاش جاری ہو';
        }
        if (typeOpts[1]) {
            if (typeOpts[1].querySelector('h4')) typeOpts[1].querySelector('h4').innerText = 'ملنے والا فرد';
            if (typeOpts[1].querySelector('p')) typeOpts[1].querySelector('p').innerText = 'کوئی غیر شناخت شدہ فرد جو ملا ہو';
        }
        if (typeOpts[2]) {
            if (typeOpts[2].querySelector('h4')) typeOpts[2].querySelector('h4').innerText = 'شہید';
            if (typeOpts[2].querySelector('p')) typeOpts[2].querySelector('p').innerText = 'وہ فرد جو خدمت کے دوران شہید ہوا ہو';
        }

        setT('.wizard-step-content[data-step="2"] h3', 'شخصی معلومات درج کریں');
        const formLabels = document.querySelectorAll('#report-wizard-form label');
        if (formLabels[0]) formLabels[0].innerText = 'شخص کا مکمل نام';
        setP('#person-name', 'نام درج کریں');
        if (formLabels[1]) formLabels[1].innerText = 'عمر (سال)';
        setP('#person-age', 'عمر درج کریں');
        if (formLabels[2]) formLabels[2].innerText = 'جنس';
        if (formLabels[3]) formLabels[3].innerText = 'شناختی کارڈ (CNIC)';
        setP('#person-cnic', 'شناختی کارڈ نمبر درج کریں');
        if (formLabels[4]) formLabels[4].innerText = 'واقعہ کی تاریخ';
        if (formLabels[5]) formLabels[5].innerText = 'واقعہ کا ضلع';
        if (formLabels[6]) formLabels[6].innerText = 'مخصوص مقام (قصبہ/گاؤں)';
        setP('#person-location', 'مقام درج کریں');
        if (formLabels[7]) formLabels[7].innerText = 'تصویر اپ لوڈ کریں';
        if (formLabels[8]) formLabels[8].innerText = 'تفصیلی معلومات / جسمانی نشانات';
        setP('#person-details', 'تفصیلات درج کریں');

        setT('.wizard-step-content[data-step="3"] h3', 'رپورٹر کی معلومات درج کریں');
        if (formLabels[9]) formLabels[9].innerText = 'آپ کا مکمل نام';
        setP('#reporter-name', 'اپنا نام درج کریں');
        if (formLabels[10]) formLabels[10].innerText = 'آپ کا رابطہ نمبر';
        setP('#reporter-phone', 'رابطہ نمبر درج کریں');
        if (formLabels[11]) formLabels[11].innerText = 'لاپتہ فرد سے رشتہ';
        setP('#reporter-relation', 'مثال کے طور پر: بھائی، والد وغیرہ');

        setT('#wizard-prev-btn span', 'پچھلا');
        setT('#wizard-next-btn span', 'اگلا');
        setT('#wizard-submit-btn span', 'رپورٹ جمع کریں');
        ...
        setT('#view-submit .view-header p', 'Fill out the multi-step form to register a new report in the portal database.');
        
        const progSteps = document.querySelectorAll('.progress-step');
        if (progSteps[0] && progSteps[0].querySelector('span')) progSteps[0].querySelector('span').innerText = 'Report Type';
        if (progSteps[1] && progSteps[1].querySelector('span')) progSteps[1].querySelector('span').innerText = 'Person Details';
        if (progSteps[2] && progSteps[2].querySelector('span')) progSteps[2].querySelector('span').innerText = 'Reporter Details';

        setT('.wizard-step-content[data-step="1"] h3', 'Select Report Type');
        const typeOpts = document.querySelectorAll('.type-option');
        if (typeOpts[0]) {
            if (typeOpts[0].querySelector('h4')) typeOpts[0].querySelector('h4').innerText = 'Missing Person';
            if (typeOpts[0].querySelector('p')) typeOpts[0].querySelector('p').innerText = 'A person who went missing and is currently actively sought';
        }
        if (typeOpts[1]) {
            if (typeOpts[1].querySelector('h4')) typeOpts[1].querySelector('h4').innerText = 'Found Person';
            if (typeOpts[1].querySelector('p')) typeOpts[1].querySelector('p').innerText = 'An unidentified person found by community or police';
        }
        if (typeOpts[2]) {
            if (typeOpts[2].querySelector('h4')) typeOpts[2].querySelector('h4').innerText = 'Shaheed / Martyr';
            if (typeOpts[2].querySelector('p')) typeOpts[2].querySelector('p').innerText = 'A person who embraced martyrdom in line of duty';
        }

        setT('.wizard-step-content[data-step="2"] h3', 'Enter Person\'s Details');
        const formLabels = document.querySelectorAll('#report-wizard-form label');
        if (formLabels[0]) formLabels[0].innerText = 'Person\'s Full Name';
        setP('#person-name', 'Enter name');
        if (formLabels[1]) formLabels[1].innerText = 'Age (Years)';
        setP('#person-age', 'Enter age');
        if (formLabels[2]) formLabels[2].innerText = 'Gender';
        if (formLabels[3]) formLabels[3].innerText = 'CNIC Number (Optional)';
        setP('#person-cnic', 'Enter CNIC (xxxxx-xxxxxxx-x)');
        if (formLabels[4]) formLabels[4].innerText = 'Date of Incident';
        if (formLabels[5]) formLabels[5].innerText = 'District of Incident';
        if (formLabels[6]) formLabels[6].innerText = 'Specific Location (Town/Village)';
        setP('#person-location', 'Enter location');
        if (formLabels[7]) formLabels[7].innerText = 'Upload Person\'s Photo';
        if (formLabels[8]) formLabels[8].innerText = 'Detailed Description / Physical Marks';
        setP('#person-details', 'Enter details');

        setT('.wizard-step-content[data-step="3"] h3', 'Enter Reporter\'s Details');
        if (formLabels[9]) formLabels[9].innerText = 'Your Full Name';
        setP('#reporter-name', 'Enter your name');
        if (formLabels[10]) formLabels[10].innerText = 'Your Contact Number';
        setP('#reporter-phone', 'Enter contact number');
        if (formLabels[11]) formLabels[11].innerText = 'Relation with Person';
        setP('#reporter-relation', 'e.g., Brother, Father, Police Officer');

        setT('#wizard-prev-btn span', 'Previous');
        setT('#wizard-next-btn span', 'Next');
        setT('#wizard-submit-btn span', 'Submit Report');
